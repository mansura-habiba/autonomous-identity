...
class AutonomousIdentity:
    ...
    def __init__(
        ...
        *,
        ...
        self._signer = signer
        self._validator = IdentityValidator(strictness=strictness)
    ...
    def run_material_action(
        ...
        *,
        ...
        if required_scope and envelope.delegations:
            allowed = [s for d in envelope.delegations for s in d.allowed_scopes]
            if required_scope not in allowed:
                raise VerificationError(f"Required scope not delegated: {required_scope}")
        self._validator.validate(envelope)
        if not self._adapter.verify(envelope):
